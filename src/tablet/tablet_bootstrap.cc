...
class TabletBootstrap {
 ...
 private:
  ...
  enum MutationApplicationStatus {
    MUTATION_NOT_FLUSHED,
    MUTATION_FLUSHED_AND_COMPACTED,
    MUTATION_FLUSHED
  };
  MutationApplicationStatus DetermineMutationApplicationStatus(const MemStoreTargetPB& target);
...
Status TabletBootstrap::PlayInsertions(WriteTransactionContext* tx_ctx,
  ...
  BOOST_FOREACH(const DecodedRowOperation& op, decoded_ops) {
    ...
    if (DetermineMutationApplicationStatus(op_result.mutated_stores(0)) != MUTATION_NOT_FLUSHED) {
      if (VLOG_IS_ON(1)) {
...
Status TabletBootstrap::PlayMutation(const MutationInput& mutation_input,
  ...
  BOOST_FOREACH(const MemStoreTargetPB& mutated_store, op_result.mutated_stores()) {
    MutationApplicationStatus status = DetermineMutationApplicationStatus(mutated_store);
    switch (status) {
      case MUTATION_NOT_FLUSHED:
        num_unflushed_stores++;
        break;
      case MUTATION_FLUSHED_AND_COMPACTED:
      case MUTATION_FLUSHED:
        if (VLOG_IS_ON(1)) {
          ...
          VLOG(1) << "Skipping mutation to " << mutated_store.ShortDebugString()
                  << " that was already "
                  << ((status == MUTATION_FLUSHED) ? "flushed" : "flushed and compacted")
...
TabletBootstrap::MutationApplicationStatus TabletBootstrap::DetermineMutationApplicationStatus(
    const MemStoreTargetPB& target) {
  if (target.has_mrs_id()) {
    ...
    if (target.mrs_id() <= tablet_->metadata()->last_durable_mrs_id()) {
      return MUTATION_FLUSHED;
    }
  } else {
    ...
    if (row_set == NULL) {
      return MUTATION_FLUSHED_AND_COMPACTED;
    }
