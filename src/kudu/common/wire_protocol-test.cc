 ...
 public:
  WireProtocolTest()
      : schema_({ ColumnSchema("string", STRING),
                  ColumnSchema("nullable_string", STRING, /* nullable=*/true),
                  ColumnSchema("int", INT32),
                  ColumnSchema("nullable_int", INT32, /* nullable=*/true),
  ...
  void FillRowBlockWithTestRows(RowBlock* block) {
    ...
    for (int i = 0; i < block->nrows(); i++) {
      ...
      CHECK(test_data_arena_.RelocateSlice(
          "hello world col0",
...
TEST_F(WireProtocolTest, TestRowBlockToRowwisePB) {
  ...
  int num_rows_serialized = SerializeRowBlock(block, nullptr, &direct, &indirect);
  ...
  pb.set_num_rows(num_rows_serialized);
...
// Create a block of rows and ensure that it can be converted to the columnar serialized
// layout.
TEST_F(WireProtocolTest, TestRowBlockToColumnarPB) {
  ...
  for (int src_row_idx = 0; src_row_idx < block.nrows(); src_row_idx++) {
    ...
    for (int c = 0; c < schema_.num_columns(); c++) {
      ...
      if (!col.is_nullable() || !row.is_null(c)) {
        if (col.type_info()->physical_type() == BINARY) {
          ...
          serialized_val = Slice(serialized_col.indirect->data() + indirect_offset,
                                 serialized_val.size());
        }
  ...
  // TODO(todd): test multiple batches in a loop
}
...
TEST_F(WireProtocolTest, TestColumnarRowBlockToPBWithPadding) {
  ...
  // Convert to PB.
  faststring direct, indirect;
  int num_rows_serialized = SerializeRowBlock(
      block, &proj_schema, &direct, &indirect, true /* pad timestamps */);
...
class WireProtocolBenchmark :
 ...
 public:
      ...
      for (int j = 0; j < benchmark_schema_.num_columns(); j++) {
        ...
        if (!is_null) {
          if (type == STRING) {
            ...
            memcpy(row.mutable_cell_ptr(j), &i, sizeof(int64_t));
          } else {
  ...
  void RunBenchmark(const BenchmarkColumnsSpec& spec,
                    double select_rate,
                    Converter* conv) {
    ...
    const int kNumTrials = kNumCellsToConvert / select_rate / spec.columns.size() / block.nrows();
...
TEST_P(WireProtocolBenchmark, TestRowBlockToRowwisePBBenchmark) {
  auto spec = std::get<0>(GetParam());
  double select_rate = std::get<1>(GetParam());
  RowwiseConverter conv;
  RunBenchmark(spec, select_rate, &conv);
}

TEST_P(WireProtocolBenchmark, TestRowBlockToColumnarPBBenchmark) {
  auto spec = std::get<0>(GetParam());
  double select_rate = std::get<1>(GetParam());

  ColumnarConverter col_conv;
  RunBenchmark(spec, select_rate, &col_conv);
}

BenchmarkColumnsSpec UniformColumns(int n_cols, DataType type, double null_fraction) {
  vector<BenchmarkColumnsSpec::Col> cols;
  for (int i = 0; i < n_cols; i++) {
    cols.push_back({type, null_fraction});
  }
  string null_str;
  if (null_fraction >= 0) {
