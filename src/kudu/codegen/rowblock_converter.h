...
class RowBlockConverterFunction : public JITWrapper {
 public:
  ...
  typedef void(*ConversionFunction)(RowBlock* block, uint8_t* dst_base,
                                    faststring* indirect);
...
class RowBlockConverter {
 public:
  ...
  void ConvertRowBlockToPB(const RowBlock& block, RowwiseRowBlockPB* pb,
    ...
    RowBlock* rb = const_cast<RowBlock*>(&block);
    size_t old_size = row_data->size();
    int num_rows = block.selection_vector()->CountSelected();
    row_data->resize(old_size + row_stride_ * num_rows);
    uint8_t* dst_base = &(*row_data)[old_size];
    function_->converter()(rb, dst_base, indirect_data);
