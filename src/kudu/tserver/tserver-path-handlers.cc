...
string TabletServerPathHandlers::IteratorStatsToHtml(const Schema& projection,
  ...
  for (size_t idx = 0; idx < stats.size(); idx++) {
    ...
    html << Substitute("<tr>"
                       ...
                       stats[idx].data_blocks_read_from_disk, // $1
                       HumanReadableNumBytes::ToString(stats[idx].bytes_read_from_disk), // $2
                       stats[idx].bytes_read_from_disk, // $3
                       HumanReadableInt::ToString(stats[idx].rows_read_from_disk), // $4
                       stats[idx].rows_read_from_disk); // $5
  }
  html << "</table>\n";
  return html.str();
}
