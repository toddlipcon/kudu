...
// and can't connect.
DEFINE_bool(client_use_domain_sockets, false,
            "Whether to try to connect to tablet servers using unix domain sockets. "
            "This will only be attempted if the server has indicated thta it is listening "
            "on such a socket.");
TAG_FLAG(client_use_domain_sockets, experimental);
...
void RemoteTabletServer::InitProxy(KuduClient* client, const StatusCallback& cb) {
  ...
  if (FLAGS_client_use_domain_sockets && unix_domain_socket_path_ &&
    ...
    if (!parse_status.ok()) {
      KLOG_EVERY_N_SECS(WARNING, 60)
          << "Tablet server " << hp.ToString() << "(" << uuid_ << ") "
          << " reported an invalid unix domain socket path " << *unix_domain_socket_path_;
      // Fall through to normal TCP path.
    } else {
      VLOG(1) << "Will try to connect to unix socket "
              << unix_socket.ToString() << " for local tserver "
              << hp.ToString();
