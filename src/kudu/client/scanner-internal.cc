...
void KuduScanBatch::Data::ExtractRows(vector<KuduRowResult>* rows) {
  ...
  // Initialize each KuduRowResult with data from the response.
  //
  // Doing this resize and array indexing turns out to be noticeably faster
  // than using reserve and push_back.
  int projected_row_size = CalculateProjectedRowSize(*projection_);
  const uint8_t* src = direct_data_.data();
  KuduRowResult* dst = &(*rows)[0];
  while (n_rows > 0) {
