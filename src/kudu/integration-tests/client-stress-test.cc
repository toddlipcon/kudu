...
// Regression test for KUDU-1104, a race in which multiple
// scanners racing to populate a cold meta cache on a shared
// Client would crash.
//
// This test creates a table with a lot of tablets (so that we
// require many round-trips to the master to populate the meta
// cache) and then starts a bunch of parallel threads to start
// scans starting at random points in the key space.
